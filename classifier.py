...
PATH_TO_TEST_LABELS = "sampleSubmission.csv"
...
def clean_text(sf):
    sf['text_clean'] = sf['text'].apply(
        lambda x: re.sub(r'[\n\t,.:;()\-\/]+', ' ', ' '.join(x)))
    sf['text_clean'] = sf['text_clean'].apply(
...
# read train and test labels from paths PATH_TO_TRAIN_LABELS and
# PATH_TO_TEST_LABELS
...
# create a TFIDF Transformer that is fit on your training data and
# transform both training and testing data
encoder = gl.feature_engineering.create(
    train, TFIDF('bow', output_column_name='tfidf'))
train = encoder.transform(train)
test = encoder.transform(test)

# train logistic regression on training data with tf-idf as features and
# predict on testing data
train = train.dropna()
model = gl.logistic_classifier.create(
    train, target='sponsored', features=['tfidf'], class_weights='auto')
